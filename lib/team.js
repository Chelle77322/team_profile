...
inquirer.prompt(this.promptAddEmployee).then (data =>{
    ...
    switch (data.role) {
        ...
        case "manager":
            this.storeEmployee.push(new manager(data.name, data.id, data.email,data.classification, data.department,data.floorLevel, data.OfficeNumber));
            this.nextEmployees();
            break;
        case "line_manager":
    ...
    generateHTML(){
        ...
        getTemplate() {
            ...
            this.storeEmployee.forEach(data => {
                ...
                var card = $('<div class="card mx-auto border-info mb-3" style="max-width: 18rem;" id = "team">' );
                ...
                var card = $('<div class="card-body text-info">');
